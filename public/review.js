    ...
    if (!json.norev) {
        ...
        if (json.length > 10) {
            for (let i = 0; i < 5; i++) {
                ...
                const profileLink = document.createElement("a");
                profileLink.href =
                    "profile?q=" + encodeURIComponent(item.USERNAME);
...
function getRandomReviews() {
    fetch("/getRandomReviews?q=" + encodeURIComponent(id))
        .then(onResponse)
...
function onJsonMyReviewLikes(json) {
    if (!json.nolike) {
        ...
        const reviews = document.querySelectorAll(".review");
        for (item of reviews) {
            const dataIndex = item.dataset.reviewid;
            if (indexs.includes(parseInt(dataIndex))) {
...
function toggleReviewHeart(event) {
    ...
    if (event.currentTarget.src.includes(emptyheart)) {
        fetch("/addReviewLike", {
            method: "post",
            body: formData,
            headers: { "X-CSRF-TOKEN": token },
